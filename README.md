...
## 資料品質檢查工具 Data quality check tool

新增 `tools/data_quality/duplicate_hourly_report.py`，可掃描 `data/*/*.csv` 並產生「同站點、同日期、同時間戳」的重複資料報告：

- `duplicate_identical.csv`：時間戳重複且數值完全一致。
- `duplicate_conflicting.csv`：時間戳重複但數值不一致。
- `duplicate_report.md`：摘要統計。
...
也可使用 GitHub Actions workflow：`.github/workflows/hourly-duplicate-report.yml`（支援手動觸發與每週排程）。
